...
// ─── Display Target Selector (reusable) ───────────────────────────────────────
const DisplayTargetSelector = ({ value, onChange }) => (
  <div>
    <label className={lbl}>
      Display Target <Req />
    </label>
    <div className="flex gap-3">
      ...
      >
        <MobileIcon /> Mobile App
      </button>
    </div>
    <p className="text-xs text-gray-400 mt-1.5">
      {value === "mobile"
        ? "Categories ke upar featured section mein dikhega (mobile app)"
        : "Website pe banner / section mein dikhega (desktop)"}
    </p>
  </div>
);
...
// CREATE FLASH SALE
// =============================================================================
function CreateFlashSale({ onBack, onCreated }) {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex gap-8">
          <div className="flex-1 space-y-5">
            {/* Display Target — most important, at top */}
            <DisplayTargetSelector
              value={form.displayTarget}
              onChange={(v) => setForm({ ...form, displayTarget: v })}
            />
          ...
          <div className="w-60 flex-shrink-0">
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              ...
              <span className="text-blue-500 font-normal">
                {form.displayTarget === "mobile" ? "1:1 (400×400 px)" : "Ratio 3:2 (600×400 px)"}
              </span>{" "}
              <Req />
            </label>
            <div
        ...
        <div className="flex items-center justify-between mt-10 pt-6 border-t border-gray-200">
          ...
          <button onClick={handleSubmit} disabled={loading}
            ...
            {loading ? <><Spinner sm /> Creating...</> : "Create Flash Sale"}
          </button>
        </div>
      </div>
    </div>
...
function EditFlashSale({ sale, onBack, onUpdated }) {
  const [form, setForm] = useState({
    ...
    displayTarget: sale.displayTarget || "web",
  });
  const [thumbnail, setThumbnail] = useState(null);
...
// =============================================================================
// FLASH SALE DETAIL (unchanged except TargetBadge in header)
// =============================================================================
function FlashSaleDetail({ saleId, onBack }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-gray-900">Flash Deal Details</h2>
            <TargetBadge target={sale.displayTarget} />
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          ...
          <div className="flex gap-3">
            <select value={selectedProductId} onChange={(e) => setSelectedProductId(e.target.value)} className={`flex-1 ${inp}`}>
              <option value="">Select Product</option>
              {availableProducts.map((p) => (
                <option key={p.id} value={p.id}>{p.name}{p.price != null ? ` — $${p.price}` : ""}</option>
              ))}
            </select>
            <button onClick={handleAddProduct} disabled={!selectedProductId || addingProduct}
                  ...
                  <tr key={product.id} className="hover:bg-gray-50/60 transition">
                    ...
                    <td className="px-6 py-4">
                      {product.thumbnail || product.image ? (
                        <img src={product.thumbnail || product.image} alt={product.name} className="w-12 h-12 object-cover rounded-lg border border-gray-100" />
                      ) : (
      ...
      {editProduct && (
        <UpdateProductModal saleId={saleId} product={editProduct} onClose={() => setEditProduct(null)} onUpdated={handleProductUpdated} />
      )}
    </div>
  );
}
...
// =============================================================================
// FLASH SALE LIST — with tabs (Web / Mobile)
// =============================================================================
function FlashSaleList({ onCreateNew, onView, onEdit }) {
  ...
  const fetchSales = async (target) => {
    setLoading(true);
    try {
      const res = await fetch(`${BASE_URL}/all?target=${target}`);
      const data = await res.json();
  ...
  useEffect(() => { fetchSales(activeTab); }, [activeTab]);
    ...
    <div className="min-h-screen bg-gray-50 px-6 py-10">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={onCreateNew}
            className="flex items-center gap-2 px-5 py-2.5 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold rounded-lg transition shadow-sm"
          >
            <PlusIcon /> Create Flash Sale
          </button>
        </div>
        ...
        <p className="text-xs text-gray-400 mb-4">
          {activeTab === "mobile"
            ? "📱 Ye flash sales mobile app mein categories ke upar featured section mein dikhti hain."
            : "🖥️ Ye flash sales website ke desktop view mein banner/section mein dikhti hain."}
        </p>

        {error && (
          <div className="mb-4 bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg px-4 py-3">⚠️ {error}</div>
        )}
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="py-20 text-center text-gray-400">
              No {activeTab} flash sales yet.{" "}
              <button onClick={onCreateNew} className="text-green-500 hover:underline font-medium">Create one!</button>
            </div>
          ) : (
            <table className="w-full">
              <thead>
              ...
              <tbody className="divide-y divide-gray-100">
                {sales.map((sale, i) => (
                  <tr key={sale.id} className="hover:bg-gray-50/60 transition">
                    ...
                    <td className="px-5 py-4 text-sm text-gray-600 whitespace-nowrap">{formatDateTime(sale.startDate, sale.startTime)}</td>
                    <td className="px-5 py-4 text-sm text-gray-600 whitespace-nowrap">{formatDateTime(sale.endDate, sale.endTime)}</td>
                    <td className="px-5 py-4">
                      <button
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => onView(sale.id)}
                          className="w-8 h-8 flex items-center justify-center rounded-lg bg-green-50 text-green-600 hover:bg-green-100 transition" title="View">
                          <EyeIcon />
                        </button>
                        <button onClick={() => onEdit(sale)}
                          className="w-8 h-8 flex items-center justify-center rounded-lg bg-blue-50 text-blue-500 hover:bg-blue-100 transition" title="Edit">
                          <EditIcon />
                        </button>
                        <button onClick={() => handleDelete(sale.id)}
                          className="w-8 h-8 flex items-center justify-center rounded-lg bg-red-50 text-red-500 hover:bg-red-100 transition" title="Delete">
                          <TrashIcon />
                        </button>
                      </div>
                    </td>
...
export default function FlashSaleApp() {
  ...
  if (page === "view") return <FlashSaleDetail saleId={selectedSaleId} onBack={goList} />;
  if (page === "create") return <CreateFlashSale onBack={goList} onCreated={goList} />;
  if (page === "edit") return <EditFlashSale sale={selectedSale} onBack={goList} onUpdated={goList} />;
